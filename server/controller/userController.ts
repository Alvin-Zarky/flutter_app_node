...
const authUserLogin: RequestHandler = asyncHandler(async(req, res, next) =>{
  ...
  const user= await User.findOne({ where:{ email: { [Op.iLike]: `%${email.toLowerCase()}%` } }}) as UserModel
  if(!user){
...
const authUserRegister: RequestHandler = asyncHandler(async(req, res, next) =>{

  const {name, email, password, confirmPassword, phoneNumber} = req.body as UserModel
...
const authGetUserProfile: RequestHandler = asyncHandler(async(req, res ,next) =>{
