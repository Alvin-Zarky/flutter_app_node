...
sequelize.sync({alter:true}).then(() =>{
  ...
  console.error(colors.red.underline(`Database Connection Error: ${err.message}`));
})

const server= app.listen(PORT, () =>{
  console.log(colors.bgCyan.bold.inverse(`Server in ${process.env.NODE_ENV} is running on port ${process.env.PORT}`));
});
